...
import { 
  Wallet, PlusCircle, ArrowRightLeft, History, TrendingUp, TrendingDown, 
  DollarSign, Landmark, Loader2, Calendar, ShieldCheck, Activity, Check 
} from 'lucide-react';

const Financeiro = () => {
  const { isAdmin } = useAdmin();
  const { 
    nortePalpites, sulPalpites, loading: dashLoading, fetchData, currentWeek 
  } = useDashboardData();
  ...
  const [bases, setBases] = useState({ banco: 271.00, casa: 143.30 });
  ...
  const fetchFinanceData = async () => {
    try {
      ...
      const { data: tData, error: tError } = await supabase.from('banca_transferencias').select('*');
      const { data: mData, error: mError } = await supabase.from('mensalidades').select('valor_pago').eq('pago', true);
      const { data: bT, error: bE } = await supabase.from('banca_transacoes').select('*');
      const { data: bP, error: bP_E } = await supabase.from('banca_particoes').select('*').eq('id', 1).maybeSingle();
      ...
      const sortedTransfers = (tData || []).sort((a,b) => new Date(b.created_at || b.data || b.id || 0) - new Date(a.created_at || a.data || a.id || 0));
      const sortedManual = (bT || []).sort((a,b) => new Date(b.created_at || b.data || b.id || 0) - new Date(a.created_at || a.data || a.id || 0));
  ...
  const manualInputsBanco = manualTransactions.filter(t => t.pago !== false && (t.tipo === 'ENTRADA' || t.tipo === 'MENSALIDADE' || t.tipo === 'PREMIO')).reduce((acc, t) => acc + Number(t.valor), 0);
  const manualOutputsBanco = manualTransactions.filter(t => t.pago !== false && (t.tipo === 'SAIDA' || t.tipo === 'MULTA' || t.tipo === 'LEVANTAMENTO')).reduce((acc, t) => acc + Math.abs(Number(t.valor)), 0);
  ...
  const handleManualTransaction = async () => {
     ...
     try {
        ...
        const novoBanco = manualMovement.tipo === 'ENTRADA' ? bases.banco + Number(manualMovement.valor) : bases.banco - Number(manualMovement.valor);
        await supabase.from('banca_particoes').update({ banco_valor: novoBanco }).eq('id', 1);
  ...
  const handleTransfer = async () => {
    ...
    try {
      const { error: tError } = await supabase.from('banca_transferencias').insert({
          origem: transferencia.de, destino: transferencia.para, valor: val, created_at: new Date()
      });
      ...
      let nB = bases.banco;
      let nC = bases.casa;
      if (transferencia.de === 'BANCO') { nB -= val; nC += val; }
      else { nB += val; nC -= val; }
      
      await supabase.from('banca_particoes').update({ banco_valor: nB, casa_valor: nC }).eq('id', 1);
  ...
  if (loading || dashLoading) return <div className="text-white text-center mt-20 animate-pulse font-black uppercase text-xs italic tracking-[0.2em]">Auditando Tesouraria Master...</div>;
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-1000 space-y-10 pb-20 px-4 max-w-lg mx-auto overflow-x-hidden">
      <div className="mb-2">
        ...
        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mt-1 ml-1 italic font-display">Semana {currentWeek} de Operações</p>
      </div>
      ...
      <div className="relative group overflow-hidden bg-slate-900 border-2 border-primary/20 rounded-[50px] p-10 shadow-2xl text-center transition-all hover:bg-slate-950">
          <p className="text-primary/40 text-[9px] font-black uppercase tracking-[0.4em] mb-4 italic">Capital Total da Liga</p>
         ...
         <div className="bg-slate-900 border-2 border-emerald-500/20 rounded-[40px] p-8 space-y-6 shadow-2xl relative overflow-hidden">
            <div className="flex justify-between items-center">
               <h3 className="text-white font-black text-xs uppercase tracking-widest italic flex items-center gap-3">
                  <TrendingUp className="text-emerald-500" size={20} /> Lançamento de Caixa
               </h3>
            ...
            <div className="grid grid-cols-2 gap-4">
               <div className="relative h-14">
                  <input type="number" placeholder="0.00" value={manualMovement.valor} onChange={e => setManualMovement({...manualMovement, valor: e.target.value})} className="w-full h-full bg-slate-950 border border-white/10 rounded-2xl px-4 text-xl font-display font-black text-white outline-none focus:border-emerald-500/30 text-center" />
                  <span className="absolute right-4 top-1/2 -translate-y-1/2 text-xs font-black text-emerald-500 opacity-50">€</span>
               </div>
               <input type="text" placeholder="Descrição..." value={manualMovement.descricao} onChange={e => setManualMovement({...manualMovement, descricao: e.target.value})} className="w-full h-14 bg-slate-950 border border-white/10 rounded-2xl px-5 text-[10px] font-black text-white outline-none focus:border-emerald-500/30 italic uppercase" />
            </div>

            <button onClick={handleManualTransaction} disabled={saving} className={`w-full h-14 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2 ${manualMovement.tipo === 'ENTRADA' ? 'bg-emerald-500 text-slate-950' : 'bg-rose-500 text-white'}`}>
               {saving ? <Loader2 className="animate-spin" /> : <><PlusCircle size={16} /> CONFIRMAR MOVIMENTO</>}
            </button>
         </div>
      )}
      ...
      {isAdmin && (
         <div className="bg-slate-950/50 border-2 border-primary/10 rounded-[40px] p-8 space-y-6">
            ...
            <div className="grid grid-cols-[1fr_30px_1fr] items-center gap-3">
               <select value={transferencia.de} onChange={e => setTransferencia({...transferencia, de: e.target.value})} className="bg-slate-900 border border-white/5 rounded-2xl h-14 px-3 text-[10px] font-black text-white outline-none">
                  <option value="BANCO">🏦 BANCO</option>
                  <option value="CASA">🎰 CASA</option>
               </select>
               <ArrowRightLeft size={16} className="text-primary/30 mx-auto" />
               <select value={transferencia.para} onChange={e => setTransferencia({...transferencia, para: e.target.value})} className="bg-slate-900 border border-white/5 rounded-2xl h-14 px-3 text-[10px] font-black text-white outline-none">
                  <option value="CASA">🎰 CASA</option>
                  <option value="BANCO">🏦 BANCO</option>
               </select>
            </div>
            ...
            <div className="relative h-14">
               <input type="number" placeholder="Montante..." className="w-full h-full bg-slate-900 border border-white/5 rounded-2xl px-6 text-lg font-black text-white outline-none text-center italic" value={transferencia.valor} onChange={e => setTransferencia({...transferencia, valor: e.target.value})} />
            </div>

            <button onClick={handleTransfer} disabled={saving} className="w-full h-14 bg-primary text-slate-950 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] active:scale-95 transition-all">
               {saving ? <Loader2 className="animate-spin mx-auto" /> : "EXECUTAR TRANSFERÊNCIA"}
            </button>
         </div>
      )}
      ...
      <div className="space-y-6">
         <div className="flex justify-between items-center px-4">
            ...
            <span className="text-[8px] font-bold text-slate-600 uppercase tracking-widest italic">Auditoria Master</span>
         </div>
         ...
         <div className="space-y-4">
            {[...transfers, ...manualTransactions]
              ...
              .map((t, idx) => {
                ...
                return (
                  <div key={t.id || idx} className="bg-slate-900 border border-white/5 rounded-[32px] p-6 flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                       <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${isTransfer ? 'bg-white/5 text-slate-500' : (t.tipo === 'SAIDA' ? 'bg-rose-500/10 text-rose-500' : 'bg-emerald-500/10 text-emerald-500')}`}>
                          {isTransfer ? <ArrowRightLeft size={18} /> : (t.tipo === 'SAIDA' ? <TrendingDown size={18} /> : <TrendingUp size={18} />)}
                       </div>
                       <div>
                          <div className="flex items-center gap-2">
                             {isTransfer ? (
                                <p className="text-[9px] font-black text-white uppercase italic">{t.origem} → {t.destino}</p>
                             ) : (
                                <p className="text-[9px] font-black text-white uppercase italic tracking-widest">{t.tipo}</p>
                             )}
                          </div>
                          <p className="text-[10px] font-black text-slate-400 italic mt-0.5 truncate max-w-[120px]">{t.descricao || 'Trf Interna'}</p>
                          <p className="text-[8px] text-slate-600 font-bold uppercase mt-1 flex items-center gap-2 tracking-widest">
                             <Calendar size={10} /> {dataValida.toLocaleDateString('pt-PT')} <span className="opacity-20">•</span> {dataValida.toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit' })}
                          </p>
                       </div>
                    </div>
                    <div className="text-right">
                       <p className={`text-lg font-display font-black italic tracking-tighter ${isTransfer ? 'text-white' : (t.tipo === 'SAIDA' ? 'text-rose-500' : 'text-emerald-500')}`}>
                          {!isTransfer && (t.tipo === 'ENTRADA' || t.tipo === 'MENSALIDADE' || t.tipo === 'PREMIO' ? '+' : '-')}
