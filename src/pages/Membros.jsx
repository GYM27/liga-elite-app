...
const MemberCard = ({ player, isAdmin, currentMonth, onComplete, editingId, setEditingId, formData, setFormData, handleSave, handleEdit }) => {
  ...
  const handlePay = async (month) => {
    ...
    try {
      ...
      const { error: mErr } = await supabase.from('mensalidades').upsert({ jogador_id: player.jogador_id, mes: month, pago: true }, { onConflict: 'jogador_id,mes' });
    ...
    } catch (err) { setStatusMsg({ t: 'ERRO CRÍTICO: ' + err.message, c: 'text-rose-500' }); }
  };
  ...
  const handleUnpay = async (e, month) => {
    ...
    try {
      const { error: mErr } = await supabase.from('mensalidades').upsert({ jogador_id: player.jogador_id, mes: month, pago: false }, { onConflict: 'jogador_id,mes' });
      if (mErr) { setStatusMsg({ t: 'ERRO: ' + mErr.message, c: 'text-rose-500' }); return; }
      const { data: bP } = await supabase.from('banca_particoes').select('banco_valor').eq('id', 1).maybeSingle();
      if (bP) await supabase.from('banca_particoes').update({ banco_valor: (Number(bP.banco_valor) || 0) - 5.0 }).eq('id', 1);
  ...
  return (
    <div className={`relative bg-slate-900 border-2 rounded-[32px] p-6 transition-all duration-300 ${IsEditing ? 'border-primary ring-4 ring-primary/5' : 'border-white/5'}`}>
      {IsEditing ? (
        <div className="space-y-6 text-left">
           ...
           <div className="space-y-4">
              <input type="text" className="w-full h-14 bg-slate-950 border border-white/10 rounded-2xl px-5 text-sm font-black text-white outline-none" value={formData.nome} onChange={(e) => setFormData({...formData, nome: e.target.value})} />
              <div className="flex gap-2 p-1 bg-slate-950 rounded-2xl border border-white/5 h-12 flex items-center justify-center">
                 <button onClick={() => setFormData({...formData, liga_atual: 'Norte'})} className={`flex-1 h-8 rounded-lg text-[9px] font-black uppercase ${formData.liga_atual === 'Norte' ? 'bg-blue-500 text-white' : 'text-slate-600'}`}>NORTE</button>
                 <button onClick={() => setFormData({...formData, liga_atual: null})} className={`flex-1 h-8 rounded-lg text-[9px] font-black uppercase ${formData.liga_atual === null ? 'bg-emerald-500 text-slate-950' : 'text-slate-600'}`}>AUTO</button>
                 <button onClick={() => setFormData({...formData, liga_atual: 'Sul'})} className={`flex-1 h-8 rounded-lg text-[9px] font-black uppercase ${formData.liga_atual === 'Sul' ? 'bg-orange-500 text-white' : 'text-slate-600'}`}>SUL</button>
              </div>
           </div>
           <div className="flex gap-3">
              <button onClick={() => handleSave(player.jogador_id)} className="flex-1 h-12 bg-primary text-slate-950 rounded-2xl font-black uppercase text-[10px] active:scale-95">Gravar</button>
              <button onClick={() => setEditingId(null)} className="px-6 h-12 bg-slate-800 text-white rounded-2xl font-black uppercase text-[10px] active:scale-95">Sair</button>
           </div>
        </div>
      ) : (
        <div className="flex flex-col gap-5 text-left">
          <div className="flex items-center gap-5">
            <div className={`w-16 h-16 rounded-[24px] overflow-hidden border-2 shadow-2xl bg-slate-950 flex-shrink-0 border-white/5 shadow-inner`}>
            ...
            <div className="min-w-0 flex-1">
               ...
               <p className={`text-[10px] font-black uppercase tracking-[0.2em] mt-0.5 text-slate-500`}>{player.liga_atual || 'Estatuto Auto'}</p>
            </div>
          </div>
          <div className="flex gap-3 pt-4 border-t border-white/5">
              <button onClick={() => handleEdit(player)} className="flex-1 h-12 flex items-center justify-center bg-white/5 border border-white/10 rounded-2xl text-slate-500 hover:text-white active:scale-90 shadow-md"><Settings2 size={22} /></button>
              <button onClick={() => { setShowAuditModal(true); setStatusMsg(null); }} className="flex-1 h-12 rounded-2xl flex items-center justify-center bg-slate-950 border border-emerald-500/20 text-emerald-500 transition-all active:scale-90 shadow-lg"><Wallet size={22} /></button>
          </div>
            ...
            <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/95 backdrop-blur-sm animate-in fade-in">
               <div className="bg-slate-900 border-2 border-primary/10 w-full max-w-sm rounded-[40px] p-8 space-y-6 shadow-2xl relative text-left overflow-hidden">
                  <div className="flex justify-between items-start border-b border-white/5 pb-4">
                     <div><h3 className="text-white font-black text-[10px] uppercase tracking-widest italic underline decoration-primary underline-offset-4">Auditória de Membro</h3><p className="text-2xl font-black text-white italic truncate mt-2 leading-tight">{player.nome}</p></div>
                     <button onClick={() => setShowAuditModal(false)} className="w-10 h-10 bg-slate-800 rounded-full flex items-center justify-center text-slate-400 group-active:scale-90"><X size={20} /></button>
                  </div>
                  ...
                  <div className="space-y-4 max-h-[40vh] overflow-y-auto pr-2 scrollbar-hide py-2">
                     <div className="space-y-3">
                        <p className="text-[9px] font-black text-rose-500 uppercase tracking-[0.2em] italic flex items-center gap-2 underline">Pendentes</p>
                        {MESES_EPOCA.map(m => {
                           const isPaid = player.historico_mensalidades?.[m];
                           if (isPaid) return null;
                           return (
                              <button key={m} onClick={() => handlePay(m)} className="w-full bg-slate-950 border border-rose-500/20 p-5 rounded-[32px] flex justify-between items-center transition-all active:scale-95 text-left">
                                 <div><p className="text-[10px] font-black text-white uppercase italic">{m}</p><p className="text-2xl font-display font-black text-rose-500">5.00€</p></div>
                        ...
                        <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.2em] italic flex items-center gap-2 underline mt-6">Histórico Pago</p>
                        {MESES_EPOCA.map(m => {
                           const isPaid = player.historico_mensalidades?.[m];
                           if (!isPaid) return null;
                           return (
                              <div key={m} className="w-full bg-emerald-500/5 border border-emerald-500/20 p-4 rounded-2xl flex justify-between items-center">
                                 <span className="text-[10px] font-black text-emerald-400 uppercase italic opacity-70">{m} ✅</span>
                                 <button onClick={(e) => handleUnpay(e, m)} className="text-rose-500 text-[10px] font-black underline flex items-center gap-1 active:scale-90"><Trash2 size={14} /> Anular</button>
                              </div>
                           );
                        })}
                     </div>
                  </div>
                  <button onClick={generateIndividualReport} className="w-full h-14 bg-emerald-500 text-slate-900 rounded-[28px] font-black uppercase text-[11px] flex items-center justify-center gap-2 active:scale-95 shadow-xl shadow-emerald-500/20 mt-4"><MessageCircle size={20} /> Enviar Comprovativo</button>
               </div>
            </div>
          )}
        </div>
