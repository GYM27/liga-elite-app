...
const BetSlip = ({ league, palpites = [], isAdmin, onToggleStatus }) => {
  ...
  const allGreen =
    palpites.length > 0 &&
    palpites.every((p) => p.resultado_individual === "GREEN");
  ...
  return (
    <div className="relative group flex flex-col h-full min-h-[580px]">
      <div
        className={`flex flex-1  border-2 rounded-[40px] overflow-hidden transition-all duration-500 flex flex-col ${statusStyles[status]}`}
      >
        {/* Header do Bilhete */}
        <div
          className={`${status === "WON" ? "bg-emerald-600" : status === "LOST" ? "bg-rose-600" : "bg-slate-900"} text-white pt-11 pb-6 px-6 relative transition-colors flex flex-col items-center justify-center`}
        >
          {/* Badge de Status Centrada */}
          <div className="absolute top-4 left-0 right-0 flex justify-center">
            <div className="flex items-center justify-center gap-1 bg-white/10 text-white px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border border-white/20 min-w-[80px]">
              {status === "WON" ? (
        ...
        <div className="flex-1 p-6 space-y-4">
          {palpites.length === 0 ? (
            <div className="h-full flex items-center justify-center text-slate-300 italic text-xs uppercase font-black tracking-widest opacity-30">
              Sem Palpites nesta Semana
            </div>
          ) : (
            palpites.map((p, i) => (
              <div
                key={i}
                onClick={() =>
                  isAdmin && onToggleStatus(p.id, p.resultado_individual)
                }
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-0.5">
                    ...
                    <p
                      className={`text-[11px] font-black uppercase leading-tight ${p.resultado_individual === "RED" ? "text-rose-400 line-through" : "text-slate-800"}`}
                    >
                      {p.equipa_casa}{" "}
                  ...
                  <div className="flex items-center gap-2 ml-4">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-tighter italic">
                      {p.aposta || "TBA"}
                      <span
                        className={`ml-1.5 not-italic border-l border-black/10 pl-1.5 ${p.resultado_individual === "RED" ? "text-rose-500 font-extrabold line-through" : p.resultado_individual === "GREEN" ? "text-emerald-600" : "text-primary"}`}
                      >
                        — {p.jogadores?.nome || "Sócio"}
                      </span>
                    </p>
                    {p.resultado_individual === "GREEN" && (
                      <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse shadow-[0_0_8px_#22c55e]"></div>
                    )}
                    {p.resultado_individual === "RED" && (
        ...
        <div
          className={`p-6 space-y-2.5 ${status === "PENDING" ? "bg-slate-50" : "bg-black/5"}`}
        >
...
const Bilhetes = () => {
  ...
  if (loading)
    return (
      <div className="text-white text-center mt-20 animate-pulse font-black uppercase tracking-widest italic font-display">
        Sintonizando Linha Temporal...
      </div>
    );

  const weekToView = selectedWeek || currentWeek;
  const weeklyPalpites = (fullHistory || []).filter(
    (p) => Number(p.semana) === Number(weekToView),
  );

  const nortePalpites = weeklyPalpites.filter(
    (p) => p.liga_no_momento?.toLowerCase() === "norte",
  );
  const sulPalpites = weeklyPalpites.filter(
  ...
  const handleToggle = async (id, currentResult) => {
    ...
    const next =
      nextStatusMap[currentResult === undefined ? null : currentResult];
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-1000 space-y-12 pb-20 px-4 max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 gap-20">
        <BetSlip
          league="Norte"
          palpites={nortePalpites}
          isAdmin={isAdmin}
          onToggleStatus={handleToggle}
        />
        <BetSlip
          league="Sul"
          palpites={sulPalpites}
          isAdmin={isAdmin}
          onToggleStatus={handleToggle}
        />
      </div>
