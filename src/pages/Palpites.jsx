...
const Palpites = () => {
  ...
  const generateReport = () => {
    ...
    // Copia para o clipboard e abre o WhatsApp
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-1000 space-y-12 pb-10 px-2 max-w-lg mx-auto text-left">
        ...
        <div className="relative group w-full max-w-xs text-left">
          ...
          >
            {(availableWeeks || []).map((w) => (
              <option key={w} value={w} className="bg-slate-900 text-white font-black">
      ...
      <div className="mb-6 flex justify-between items-center px-2 pt-6">
        <h2 className="text-3xl font-display font-black text-white tracking-tight uppercase italic flex items-center gap-3">
          ...
          <span className="text-primary tracking-widest uppercase">Palpites</span>
        </h2>
      </div>
      ...
      <div className="space-y-16">
        ...
        <section className="space-y-8">
          <div className="flex justify-between items-center px-4 border-l-4 border-blue-400 py-1">
            ...
            {isAdmin && Number(viewWeek) === Number(currentWeek) && (
              <button
                onClick={() => setOcrModal({ open: true, league: "Liga Norte", players: jogadoresNorte })}
        ...
        <section className="space-y-8 pt-10 border-t border-white/5">
          <div className="flex justify-between items-center px-4 border-l-4 border-orange-400 py-1">
            ...
            {isAdmin && Number(viewWeek) === Number(currentWeek) && (
              <button
                onClick={() => setOcrModal({ open: true, league: "Liga Sul", players: jogadoresSul })}
    ...
    <div className="space-y-1 relative" ref={wrapperRef}>
      ...
      <div className="relative h-14">
        <input
          ...
          onFocus={() => { if (value.length >= 3) setShow(true); }}
...
const PlayerCard = ({ jogador, isAdmin, initialData, week, rank, onComplete, teamsPool, updateResult }) => {
  ...
  return (
    <div className={`relative overflow-hidden bg-slate-900 border-2 p-6 rounded-[40px] shadow-2xl transition-all duration-500 min-h-[460px] flex flex-col justify-between ${isPending ? "border-amber-500/20 bg-amber-500/[0.02]" : "border-white/5"}`}>
        ...
        <div className={!canEdit ? "opacity-50 pointer-events-none" : ""}>
          ...
          <div className="space-y-1">
            ...
            <div className="grid grid-cols-[1fr_85px] gap-3 h-14">
              <input type="text" placeholder="Qual a aposta?" value={details.ap} onChange={(e) => setDetails({ ...details, ap: e.target.value })} className="w-full h-full bg-slate-950 border border-white/5 rounded-2xl px-5 text-xs font-black text-white focus:outline-none uppercase shadow-inner transition-all placeholder:text-slate-800" />
              <input type="text" placeholder="1.80" value={details.odd} onChange={(e) => setDetails({ ...details, odd: e.target.value })} className="w-full h-full bg-slate-950 border border-white/5 rounded-2xl px-2 text-xs font-black text-primary text-center focus:outline-none shadow-inner transition-all" />
            </div>
          </div>
        </div>
        {!canEdit && (
          <div className="flex items-center gap-2 px-4 py-3 bg-white/5 border border-white/10 rounded-2xl animate-in fade-in duration-500">
            ...
            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-relaxed">Bilhete Bloqueado. Contacta o Comandante.</p>
          </div>
        )}
      </div>
      ...
      <div className="pt-8">
        {canEdit ? (
          <button onClick={() => saveToDB()} disabled={saving} className="w-full h-16 rounded-[28px] font-black text-[11px] uppercase tracking-[0.3em] italic bg-primary text-slate-950 shadow-2xl active:scale-95 transition-all outline-none border-b-4 border-black/20">{saving ? "A GUARDAR..." : "Guardar Palpite 🔥"}</button>
        ) : (
          <div className="w-full h-16 rounded-[28px] font-black text-[11px] uppercase tracking-[0.3em] italic bg-slate-900 border-2 border-white/5 text-slate-600 flex items-center justify-center gap-3"><Check size={18} className="text-emerald-500/30" /> Palpite Registado</div>
        )}
      </div>
    </div>
  );
